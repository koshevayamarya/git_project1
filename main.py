...
class Funny_Student():
    def __init__(self):
        ...
        self.student_image = pygame.image.load("hero.png")
    ...
    def run(self):
        while True:
            ...
            for position2 in self.two_pos:
                if abs(position2[0] - self.student_pos[0]) <= self.student_size[0]/2:
                    if abs(position2[1] - self.student_pos[1]) <= self.student_size[1]/2:
                        message_scr = self.font.render(f"конец игры! Вы получили {self.score} пятерок", True, (255, 0, 0))
                        self.screen.blit(message_scr, (self.screen_width // 2 - message_scr.get_width() // 2,
            ...
            score_scr = self.font.render(f"Ваш счёт: {self.score} пятерок", True, (255, 255, 255))
